...
class Openpos_Core
{
    ...
    public function getReportRanges($duration,$start = false,$end = false,$time_offset_mins = 0,$limit_time = true){
    ...
    function search_customer_name($term,$limit = 5){
        ...
        $prefix = $wpdb->prefix;
        $sql = "SELECT t1.* FROM `".$prefix ."usermeta` t1  LEFT JOIN `".$prefix ."usermeta` t2 ON ( t1.user_id = t2.user_id AND CONCAT(t1.meta_value,' ',t2.meta_value) like '%".$search_string."%') WHERE ";
        ...
        if(empty($results))
        {
            $sql = "SELECT t1.* FROM `".$prefix ."usermeta` t1  LEFT JOIN `".$prefix ."usermeta` t2 ON ( t1.user_id = t2.user_id AND CONCAT(t1.meta_value,' ',t2.meta_value) like '%".$search_string."%') WHERE t1.meta_key = 'billing_first_name' AND t2.meta_key = 'billing_last_name' LIMIT 0,".$limit;
            $results = $wpdb->get_results($sql);
        }
        if(empty($results))
        {
            $sql = "SELECT t1.* FROM `".$prefix ."usermeta` t1  LEFT JOIN `".$prefix ."usermeta` t2 ON ( t1.user_id = t2.user_id AND CONCAT(t1.meta_value,' ',t2.meta_value) like '%".$search_string."%') WHERE t1.meta_key = 'shipping_first_name' AND t2.meta_key = 'shipping_last_name' LIMIT 0,".$limit;
            $results = $wpdb->get_results($sql);
        }
        $users = array();
    ...
    function search_customer_lookup_name($search_string,$limit = 10){
        ...
        $sql = "SELECT * FROM `".$customer_lookup_table ."` WHERE LOWER(CONCAT(first_name,' ',last_name)) like '%".$search_string."%' LIMIT 0,".$limit;
    ...
    function get_kitchen_url($wareshouse_id = 0){
    ...
    function get_bill_url($register_id = 0){
    ...
    function enable_hpos(){
        return  apply_filters('op_enable_hpos',false);
    }
